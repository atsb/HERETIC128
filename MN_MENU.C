...
#if (APPVER_HERETICREV < AV_HR_TIC12) // VERSIONS RESETORATION: Helper macros
#define APPVER_MENUITEM_RETTYPE void
#define APPVER_MENUITEM_RET(x) return;
#else
#define APPVER_MENUITEM_RETTYPE boolean
#define APPVER_MENUITEM_RET(x) return (x);
#endif
...
typedef struct
{
	ItemType_t type;
	char *text;
	APPVER_MENUITEM_RETTYPE (*func)(int option);
	int option;
	MenuType_t menu;
} MenuItem_t;
...
static APPVER_MENUITEM_RETTYPE SCNetCheck(int option);
#endif
static APPVER_MENUITEM_RETTYPE SCQuitGame(int option);
static APPVER_MENUITEM_RETTYPE SCEpisode(int option);
static APPVER_MENUITEM_RETTYPE SCSkill(int option);
static APPVER_MENUITEM_RETTYPE SCMouseSensi(int option);
static APPVER_MENUITEM_RETTYPE SCSfxVolume(int option);
static APPVER_MENUITEM_RETTYPE SCMusicVolume(int option);
static APPVER_MENUITEM_RETTYPE SCScreenSize(int option);
static APPVER_MENUITEM_RETTYPE SCLoadGame(int option);
static APPVER_MENUITEM_RETTYPE SCSaveGame(int option);
static APPVER_MENUITEM_RETTYPE SCMessages(int option);
static APPVER_MENUITEM_RETTYPE SCEndGame(int option);
static APPVER_MENUITEM_RETTYPE SCInfo(int option);
...
{
#if (APPVER_HERETICREV < AV_HR_TIC12)
	{ ITT_SETMENU, "NEW GAME", NULL, 0, MENU_EPISODE },
#else
...
char *QuitEndMsg[] =
{
	...
	"ARE YOU SURE YOU WANT TO QUICKSAVE?",
	"ARE YOU SURE YOU WANT TO QUICKLOAD?",
#else
...
#if (APPVER_HERETICREV < AV_HR_TIC10S)
	if(gamestate == GS_LEVEL && !(leveltime&16))
	{
		MN_DrTextA("BETA", viewwindowx+viewwidth-40, viewwindowy+7);
	}
#endif
	if(MenuActive == false)
	{
...
#endif // APPVER_HERETICREV >= AV_HR_TIC10S
...
static APPVER_MENUITEM_RETTYPE SCInfo(int option)
{
#if (APPVER_HERETICREV >= AV_HR_TIC10R)
	InfoType = 1;
#endif
#if (APPVER_HERETICREV >= AV_HR_TIC10S)
...
boolean MN_Responder(event_t *event)
{
	...
	{ // Editing file names
#if (APPVER_HERETICREV >= AV_HR_TIC12) // VERSIONS RESTORATION: Moved code (and rets+memset added)
		...
		if(slotptr < SLOTTEXTLEN && key != KEY_BACKSPACE)
		{
			...
			{
#if (APPVER_HERETICREV < AV_HR_TIC12)
				SlotText[currentSlot][slotptr] = key-32;
				SlotText[currentSlot][++slotptr] = ASCII_CURSOR;
#else
				*textBuffer++ = key-32;
				*textBuffer = ASCII_CURSOR;
				slotptr++;
			...
			{
#if (APPVER_HERETICREV < AV_HR_TIC12)
				SlotText[currentSlot][slotptr] = key;
				SlotText[currentSlot][++slotptr] = ASCII_CURSOR;
#else
				*textBuffer++ = key;
				*textBuffer = ASCII_CURSOR;
...
#if (APPVER_HERETICREV >= AV_HR_TIC12)
		return(true);
#else // VERSIONS RESTORATION: Moved code (and rets+memset added)
		if(key == KEY_BACKSPACE)
		{
			if(slotptr)
			{
				SlotText[currentSlot][slotptr] = 0;
				SlotText[currentSlot][--slotptr] = ASCII_CURSOR;
			}
		}
		if(key == KEY_ESCAPE)
...
#else // VERSIONS RESTORATION: Uncomment for shareware v1.0
