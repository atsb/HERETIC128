...
#if (APPVER_HERETICREV < AV_HR_TIC12) // VERSION RESTORATION: Unused A_Pain clone
void A_UnusedPain(mobj_t *actor)
{
	if(actor->info->painsound)
	{
		S_StartSound(actor, actor->info->painsound);
...
#if (APPVER_HERETICREV < AV_HR_TIC12) // VERSION RESTORATION: Another unused function
void A_UnusedAction(mobj_t *actor)
{
	if(actor->target)
	{
		A_FaceTarget(actor);
		if(P_CheckMeleeRange(actor))
		{
			P_DamageMobj(actor->target, actor, actor, 12+4*(P_Random()&7));
		}
	}
}
#endif
...
void A_BossDeath(mobj_t *actor)
{
	...
	line_t dummyLine;
#if (APPVER_HERETICREV < AV_HR_TIC10S)
	static mobjtype_t bossType[3] =
#elif (APPVER_HERETICREV < AV_HR_TIC12)
	static mobjtype_t bossType[4] =
#else
...
{
#if (APPVER_HERETICREV < AV_HR_TIC12) // VERSIONS RESTORATION: Uncomment for v1.0
