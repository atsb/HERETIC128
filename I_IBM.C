...
#if (APPVER_HERETICREV < AV_HR_TIC12)
						Z_ChangeTag(lumpcache[S_sfx[i].lumpnum], PU_CACHE);
#else
...
#if (APPVER_HERETICREV < AV_HR_TIC10S)
	tprintf("\nPush the joystick to the UPPER LEFT corner and press button 1:",0);
#else
...
#if (APPVER_HERETICREV < AV_HR_TIC10S)
	tprintf("\nPush the joystick to the LOWER RIGHT corner and press button 1:",0);
#else
...
void I_Init (void)
{
	...
	I_StartupMouse();
#if (APPVER_HERETICREV < AV_HR_TIC10S) // Uncomment for the beta
