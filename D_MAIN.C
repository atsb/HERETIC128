...
#if (APPVER_HERETICREV < AV_HR_TIC10S)
boolean altpal;				// checkparm to use an alternate palette routine
#endif
...
void D_PageDrawer(void)
{
	...
	memcpy(screen, W_CacheLumpName(pagename, PU_CACHE), SCREENWIDTH*SCREENHEIGHT);
#else
...
void D_AddFile(char *file)
{
	...
	strcpy(new, file);
#if (APPVER_HERETICREV < AV_HR_TIC10S) // text is set but unused
	sprintf(text, "Adding external file %s.\n", new);
#else
...
#else
#if (APPVER_HERETICREV < AV_HR_TIC12)
	progress = ((float)thermCurrent/thermMax)*50+2;
#else
...
#if (APPVER_HERETICREV >= AV_HR_TIC12)
	CheckAbortStartup();
#elif (defined __WATCOMC__) // VERSIONS RESTORATION: The function body itself
...
#if (APPVER_HERETICREV < AV_HR_TIC10S)
	p = M_CheckParm("-pubmap");
	if(debugmode && p && p < myargc-2)
	{
		e = myargv[p+1][0];
		m = myargv[p+2][0];
		sprintf(file, "i:\\id\\druid\\test\\E%cM%c.wad", e, m);
		D_AddFile(file);
		printf("PUBMAP: Episode %c, Map %c.\n", e, m);
		startepisode = e-'0';
		startmap = m-'0';
		autostart = true;
	}
...
//
	...
	if (autostart)
	{
		...
		sprintf(temp, "Warp to Episode %d, Map %d, Skill %d ",
			startepisode, startmap, startskill);
#else
		sprintf(temp, "Warp to Episode %d, Map %d, Skill %d ",
...
//
...
#elif (APPVER_HERETICREV >= AV_HR_TIC10S)
	if(autostart || netgame)
	{
		...
		else if ((startepisode != 4) || (startmap != 1))
		{
			if (startepisode < 1)
				startepisode = 1;
			if (startepisode > 3)
