...
boolean F_Responder (event_t *event)
{
	...
	{ // we're showing the water pic, make any key kick to demo mode
		finalestage++;
#if (APPVER_HERETICREV >= AV_HR_TIC12) // VERSIONS RESTORATION: Moved from F_DrawUnderwater
...
void F_DrawUnderwater(void)
{
	...
	switch(finalestage)
	{
		case 1:
			...
			break;
		case 2:
#if (APPVER_HERETICREV < AV_HR_TIC12) // VERSIONS RESTORATION: Mostly moved to F_Responder
			memset((byte *)0xa0000, 0, SCREENWIDTH*SCREENHEIGHT);
			memset(screen, 0, SCREENWIDTH*SCREENHEIGHT);
			I_SetPalette(W_CacheLumpName("PLAYPAL", PU_CACHE));
...
void F_Drawer(void)
{
	...
	else
	{
		switch (gameepisode)
		{
			case 1:
#if (APPVER_HERETICREV < AV_HR_TIC12)
				V_DrawPatch(0, 0, W_CacheLumpName("ORDER", PU_CACHE));
#else
				if(shareware)
				{
