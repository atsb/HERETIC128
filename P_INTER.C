...
void P_SetDormantArtifact(mobj_t *arti)
{
	...
	if(netgame && (arti->type != MT_ARTIINVULNERABILITY)
...
void P_HideSpecialThing(mobj_t *thing)
{
	...
	thing->z = thing->ceilingz + 4*FRACUNIT;
	thing->flags &= ~(MF_SPECIAL|MF_NOGRAVITY);
...
{
#if (APPVER_HERETICREV >= AV_HR_TIC10S) && (APPVER_HERETICREV < AV_HR_TIC12)
	subsector_t *subsec;
#endif
	...
	thing->z = thing->floorz = subsec->sector->floorheight;
	thing->ceilingz = subsec->sector->ceilingheight;
