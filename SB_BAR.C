...
#if (APPVER_HERETICREV < AV_HR_TIC12) // For ordering of global variables
...
void SB_Init(void)
{
	...
	PatchARTIBOX = W_CacheLumpName("ARTIBOX", PU_STATIC);
#endif
...
static void CheatArtifact3Func(player_t *player, Cheat_t *cheat)
{
	...
	{ // All artifacts
		...
		&& count > 0 && count < 10)
#else // APPVER_HERETICREV < AV_HR_TIC10S
	if(type > arti_none && type < NUMARTIFACTS
	   && count > 0 && count < 10)
#endif
	{
...
static void CheatWarpFunc(player_t *player, Cheat_t *cheat)
{
	...
	map = cheat->args[1]-'0';
#if (APPVER_HERETICREV < AV_HR_TIC12)
	if ((episode <= 0) || (episode >= 4) || (map <= 0) || (map >= 10))
	{
#if (APPVER_HERETICREV >= AV_HR_TIC10S)
		if ((episode != 4) || (map != 1))
#endif
